# מדריך התקנה והגדרה - Web Template
...
## 10. הגדרת מפתחות אימות (Auth Keys)
...
תראו בטרמינל: `http://localhost:3000`

לחצו עליו עם `Ctrl+Click` (או `Cmd+Click` ב-Mac) - זה יפתח את הדפדפן עם האפליקציה שלכם!
...
- ודאו שהרצתם `bunx convex dev` ושנוצר קובץ `.env.local`
